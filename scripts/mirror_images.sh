...
grep -RNIo --include='*.html' 'src="https\?://[^"\)]\+"' "$ROOT" | sed -E 's/.*src="([^"]+)".*/\1/' > "$URLS_ALL" || true
grep -RNIo --include='*.html' 'data-resize-src="https\?://[^"\)]\+"' "$ROOT" | sed -E 's/.*data-resize-src="([^"]+)".*/\1/' >> "$URLS_ALL" || true
grep -RNIo --include='*.{html,css}' 'url\((https\?://[^\)]\+)\)' "$ROOT" | sed -E 's/.*url\(([^\)]+)\).*/\1/' >> "$URLS_ALL" || true
...
# 3) Download each with high-res attempts (raw, s0, w3000, w2000, w1024)
while IFS=$'\t' read -r orig fname raw; do
  dest="$OUTDIR/$fname"
  if [[ -f "$dest" ]]; then continue; fi
  for try in "$raw" "${raw}=s0" "${raw}=w3000" "${raw}=w2000" "${raw}=w1024"; do
